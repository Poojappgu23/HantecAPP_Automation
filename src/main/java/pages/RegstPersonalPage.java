...
public class RegstPersonalPage {
	ExtentTest test;

	// constructor
	public RegstPersonalPage(WebDriver driver) {
		this.driver = driver;
	...
	@FindBy(css = "//a[contains(text(),'Next')]")
	...
	public void clickPersonalAccount(ExtentTest test) {
		...
		if (OpenRegisteration_btn.isDisplayed()) {
			...
			test.log(Status.FAIL, "couldn't click on button -OPEN AN ACCOUNT, please check error message");
			Assert.fail("couldn't click on button -OPEN AN ACCOUNT, please check error message");
		}
	}
	...
	public void DemoFirstName(String firstName, ExtentTest test) {
		try {
			...
			test.log(Status.FAIL, "couldn't Entered First name" + e);
			Assert.fail("couldn't Entered First name" + e);
		}
	}
	...
	public void DemoLastName(String lastName, ExtentTest test) {
		try {
			...
			// TODO: handle exception
			// System.out.println(e);
			test.log(Status.FAIL, "couldn't Entered Last name" + e);
			Assert.fail("couldn't Entered Last name" + e);
		}
	...
	public void DemoEmailId(String email, ExtentTest test) {
		try {
			...
			// TODO: handle exception
			// System.out.println(e);
			test.log(Status.FAIL, "couldn't Entered email ID " + e);
			Assert.fail("couldn't Entered email ID " + e);
		}
	}
	...
	public void EnterPhoneNumberWithCountryCode(String countryCode, String phoneNumber, ExtentTest test) {
		...
		try {
			...
			e.printStackTrace();
			System.out.println(e);
			test.log(Status.FAIL, "couldn't Entered Phone number " + e);
			Assert.fail("couldn't Entered Phone number" + e);
		}
	...
	public void SelectCountyFromList(ExtentTest test) {
		...
		try {
			...
			// TODO: handle exception
			// System.out.println(e);
			test.log(Status.FAIL, "couldn't Selected country from DropDown " + e);
			Assert.fail("couldn't Selected country from DropDown " + e);
		}
	}
	...
	public void EnterPassword(String Password, ExtentTest test) {
		try {
			...
			// TODO: handle exception
			// System.out.println(e);
			test.log(Status.FAIL, "couldn't Entered Password  " + e);
			Assert.fail("couldn't Entered Password  " + e);
	...
	public void PersonalApplicationRegistration(ExtentTest test) {
		...
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
		WebElement startApplicationPrg = wait.until(ExpectedConditions.elementToBeClickable(startApplication));
		...
		if (actualUrl.equals(expectedUrl)) {
			...
			new WebDriverWait(driver, Duration.ofSeconds(60))
					.until(ExpectedConditions.visibilityOf(birthDay));
		...
		test.log(Status.PASS, "clicked on Submitted Request");
		Assert.assertTrue(true);
	...
	public void DateOfBirth(String DOB, ExtentTest test) {
		birthDay.sendKeys(DOB);
	}

	public void MonthOfBirth(String MOB, ExtentTest test) {
		birthMonth.sendKeys(MOB);
	}

	public void YearOfBirth(String YOB, ExtentTest test) {
		birthYear.sendKeys(YOB);
	}

	public void Address(String Addrs, ExtentTest test) {
		birthYear.sendKeys(Addrs);
	}

	public void City(String city, ExtentTest test) {
		birthYear.sendKeys(city);
	}

	public void zip(String zip, ExtentTest test) {
		birthYear.sendKeys(zip);
	}

	public void NextButton(ExtentTest test) {
		NextButton.click();
	}
